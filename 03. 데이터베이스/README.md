...
**Q1. 정규화를 하면 데이터 무결성이 좋아지는데, 왜 실무에서는 비정규화(Denormalization)를 하나요?** </br>

A1: 정규화는 데이터 중복을 제거하고 무결성을 보장하지만, 테이블이 잘게 쪼개지면서 데이터를 조회할 때 많은 JOIN 연산이 발생하게 됩니다. </br>
이는 대규모 트래픽이 발생하는 서비스에서 성능 저하의 원인이 됩니다. </br>
따라서 조회 성능 향상이 데이터 무결성보다 우선시되는 특정 상황(예: 통계 데이터, 빈번한 상세 페이지 조회 등)에서는 </br>
의도적으로 데이터를 중복 저장하는 비정규화를 수행합니다. </br>

💡 한 끗 더: "정규화와 비정규화 사이의 **트레이드 오프(Trade-off)**를 이해하고 상황에 맞는 설계를 하는 것이 중요하다고 생각합니다." </br>

</br>

**Q2. 인덱스(Index)를 생성하면 조회 속도가 빨라지는데, 모든 컬럼에 인덱스를 걸지 않는 이유는 무엇인가요?** </br>

A2: 인덱스는 조회를 빠르게 해주지만, CUD(Create, Update, Delete) 작업 시에는 인덱스도 매번 새로 정렬하고 갱신해야 하므로 쓰기 성능이 저하됩니다. </br>
또한 인덱스 자체가 별도의 저장 공간을 차지하므로 남발할 경우 메모리 낭비가 발생합니다. </br>
따라서 카디널리티(Cardinality)가 높고(중복도가 낮고), WHERE 절이나 JOIN에 자주 사용되는 컬럼 위주로 전략적으로 생성해야 합니다. </br>
 
💡 한 끗 더: "데이터의 분포도와 쿼리 사용 빈도를 분석하여 최적의 인덱스를 설계하는 능력이 필요합니다." </br>

</br>

**Q3. 트랜잭션의 격리 수준(Isolation Level) 중 'REPEATABLE READ'에서 발생할 수 있는 'Phantom Read'란 무엇인가요?** </br>

A3: Phantom Read는 한 트랜잭션 내에서 동일한 쿼리를 두 번 실행했을 때, 첫 번째 쿼리에서 없던 '유령(Phantom) 레코드'가 </br>
두 번째 쿼리에서 나타나는 현상입니다. </br>
이는 다른 트랜잭션이 그 사이에 새로운 데이터를 INSERT했기 때문에 발생합니다.</br>
MySQL의 InnoDB 엔진은 이를 방지하기 위해 Next-Key Lock 등을 사용하지만, 표준 이론상으로는 SERIALIZABLE 수준에서만 완전히 방지됩니다.

💡 한 끗 더: "실무에서는 성능을 위해 보통 READ COMMITTED나 REPEATABLE READ를 기본으로 사용하며, 특수한 경우에만 락을 직접 제어합니다." </br>
